...
void insert_at_SAL(PersonStaticArrayList *list, Person p, int index){
	int x;
	if(list->count!=MAX_LIST && index < list->count){
...
void delete_by_sex_SAL(PersonStaticArrayList *list, char sex){ // all ocurrences
	int x, shift;
	for(x=0, shift = 0;x<list->count;x++){
		if(list->data[x].sex == sex){
			shift--;
		}
		list->data[x+1+shift] = list->data[x+1];
	}
	list->count = list->count+shift;
}
...
void insert_at_CBL(VSpace *vs, PersonCusorBasedList *list, Person p, int index){
	...
	if(temp!=-1){
		for(
			x=list, count = 0; 
			*x!=-1 && vs->data[*x].next != -1 && count<index; 
			x = &vs->data[*x].next, count++
		){}
		vs->data[temp].elem=p;
		vs->data[temp].next=*x;
		*x = temp;
